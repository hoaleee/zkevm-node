...
func (c *Client) AddCommitment(ctx context.Context, batchNum uint64, blobID []byte, dbTx pgx.Tx) error {

	// add to storage
	err := c.storage.AddCommitment(ctx, batchNum, blobID, dbTx)
	if err != nil {
		err := fmt.Errorf("failed to add tx to get monitored: %w", err)
		log.Errorf(err.Error())
		return err
	...
	mTxLog := log.WithFields("batchNum", batchNum, "celestiaBlobID", hex.EncodeToHex(blobID))
