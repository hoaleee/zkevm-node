...
// Add persist a monitored tx
func (s *PostgresStorage) AddCommitment(ctx context.Context, batchNum uint64, blobID []byte, dbTx pgx.Tx) error {
	conn := s.dbConn(dbTx)
	cmd := `
        INSERT INTO state.blobs (batch_num, blob_id)
                                 VALUES ($1, $2)`

	_, err := conn.Exec(ctx, cmd, batchNum, blobID)

	if err != nil {
		if pgErr, ok := err.(*pgconn.PgError); ok && pgErr.ConstraintName == "blobs_pkey" {
			log.Warn(`Already store empty blob! new data: `, blobID)
			return nil
		} else {
			return err
		}
