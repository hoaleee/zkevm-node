...
import (
	...
	"github.com/hoaleee/zkevm-node/etherman/etherscan"
	"github.com/hoaleee/zkevm-node/etherman/ethgasstation"
	"github.com/hoaleee/zkevm-node/etherman/metrics"
...
func NewClient(cfg Config, l1Config L1Config) (*Client, error) {
	...
	log.Info("CDKValidiumAddr: ", l1Config.ZkEVMAddr)
	// Create smc clients
...
func (etherMan *Client) readEvents(ctx context.Context, query ethereum.FilterQuery) ([]Block, map[common.Hash][]Order, error) {
	...
	for _, vLog := range logs {
		log.Debug("Number: ", vLog.BlockNumber, " Hash ", vLog.BlockHash)
