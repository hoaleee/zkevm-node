...
import (
	...
	stateMetrics "github.com/hoaleee/zkevm-node/state/metrics"
...
func NewSynchronizer(
	...
	metrics.Register()
	celestiaDa, err := celestia.NewCelestiaDA(celestia.CelestiaConfig{
		Rpc:         cfg.CelestiaDARPC,
		NamespaceId: cfg.CelestiaDANamespaceId,
	})
...
func (s *ClientSynchronizer) processSequenceBatches(sequencedBatches []etherman.SequencedBatch, blockNumber uint64, dbTx pgx.Tx) error {
	...
	for _, sbatch := range sequencedBatches {
		...
		var err error
		log.Debug("xxx: read batch number: ", sbatch.BatchNumber)
		blobID, err := s.state.GetBatchL2BlobByNumber(s.ctx, sbatch.BatchNumber, nil)
		log.Debug("xxx: db query: blobId :", blobID, "error: ", err)
		if len(blobID) > 0 && err == nil {
			batchL2Data, err = s.getBlobData(blobID)
			if err != nil {
